...
**Date/period spines (`generate_series` or the engine's equivalent) must never hardcode the backfill end date as a permanent upper bound.** Read Section 10's **Ongoing Horizon** (Q9) in `data-product-plan.md`: if it says the range extends forward, the spine's upper bound must be a dynamic expression (`CURRENT_DATE`/`CURRENT_DATE()`/`GETDATE()` per the engine's date-function reference in `engines/<engine>.md`, truncated to the model's grain), never a literal date. The Section 10 **Backfill** start/end dates describe the initial `--restate-model --start/--end` load range only (see Stage 4) — they are not model SQL constants. Only hardcode a literal upper bound when Section 10 explicitly says the time range is a closed/fixed historical snapshot. If Ongoing Horizon is missing from the spec, ask the user before hardcoding anything — do not silently assume a fixed range.
...
| Date/period spine horizon | Any `generate_series`/engine-equivalent spine in a gold model matches Section 10's Ongoing Horizon — dynamic bound if it extends forward, literal only if the spec says fixed/closed | Read the gold model SQL; compare its upper bound against Section 10 |
...
- Semantics — behavior: the canonical typing block is `behavior:` (NOT `semantic_config:` — that name and `behaviour:` still work but log a deprecation warning, so always emit `behavior`). Dimensions: `behavior.type` = identifier | categorical. Measures: `behavior.type` = simple | flow | stock | ratio. `stock` REQUIRES `time_dimension`, `period_treatment`, AND `period_grain` (e.g. `period_grain: day`). `ratio` uses `numerator` + `denominator` (measure names, direct children of `behavior`, siblings of `type`), type `number`, and NO `expression`. DOC/CLI SKEW: some installed CLIs reject this `ratio` shape (they require a `type: number` measure to carry a non-empty `expression`); if the plan rejects it, drop the `ratio` behavior and express the ratio as filtered `count`/`sum` measures, dividing downstream (metric layer or BI) — the division step is mandatory, and the component measures need a caveat marking them as ratio components so they don't read as ordinary additive measures (see the fuller note under Group C pre-check step 6).
