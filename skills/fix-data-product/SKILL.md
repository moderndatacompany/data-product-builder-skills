...
- **Status badges**: `Review run` (complete/partial), `Modeling readiness` (e.g. `changes_required`), `Cost posture` (e.g. `watch`). These are three **independent** signals — there is no single fixed-enum "Decision" line (`Approved`/`Redesign required`/etc.) anywhere in the report; don't invent one, and don't infer an overall verdict from the finding count (a blocker existing doesn't make the run `partial` — `Review run` status is reported independently of findings).
