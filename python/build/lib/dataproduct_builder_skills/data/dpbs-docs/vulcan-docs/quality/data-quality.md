...
{% hint style="info" %}
**Migrating from an older project?**

Column profiling used to be configurable via a `profiles` property inside a `MODEL(...)` block. That's deprecated — profiling now lives exclusively under `profiles:` in a `kind: dq` file, as shown throughout this page.
{% endhint %}
...
`kind: dq` declares the file as a Data Quality rule pack (the document root also accepts `type: dq` as a case-insensitive alias). `name` identifies the pack. `depends_on` ties every rule in the file to a single model.
