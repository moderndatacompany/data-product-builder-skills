...
* `trino`
* `mssql` (SQL Server)
* `fabric` (Microsoft Fabric, built on the `mssql` adapter)
...
Databricks now also works as a depot connection: use `type: databricks` with a depot `address`, and Vulcan resolves credentials from the DataOS secret projection.

{% hint style="warning" %}
`vulcan create_deploy_yaml` can't infer the execution engine from a `depot` connection's address. Set `deployment.engine` explicitly in `config.yaml` when your gateway connection is `type: depot`; for direct connections (`mssql`, `postgres`, `fabric`, and so on) `engine` is derived automatically. See [Deployment](configurations/README.md#deployment).
{% endhint %}
...
You can create this file manually, or generate it with the Vulcan CLI:

```bash
vulcan create_deploy_yaml
```

`create_deploy_yaml` is a deterministic renderer, not a starter template: every value in the output is derived from your project, defaulted, or set explicitly in the `deployment:` block in `config.yaml` — there are no placeholders left to fill in by hand. See [Deployment](configurations/README.md#deployment) for the `deployment:` field reference. Pipe the result straight into `dataos-ctl` instead of writing a file first:
...
For Trino-backed data products, a `/metrics` endpoint exposes engine-level query metrics for observability and monitoring integration.
...
* `orders-analyticsv1-api-*` and `servicev2alpha...api...` entries belong to **api** (check `*-main` for API and GraphQL logs — GraphQL is served in-process at `/semantic/graphql` — and `*-sc-2` for MySQL)
...
| API and GraphQL logs      | `<name>-api`          | `main` |
...
*   **API replicas and sidecars.** The API pod has multiple containers, each with its own logs:
    ...
    | ------------------ | ---------- | ----------------------------------------------- |
    | Main API container | `*-main`   | Core API/service behavior, including GraphQL (served in-process at `/semantic/graphql`) |
