...
| `column_mask_expressions` | SQL substitutions for columns masked by policies                                                                          |      `dict`      |     N    |
| `column_classifications` | Governance/catalog sensitivity labels per column                                                                           |      `dict`      |     N    |
...
### column\_mask\_expressions

Define the SQL replacement expression for columns that can be masked by a [policy](../../policies.md). Policies decide which groups and which columns are masked; `column_mask_expressions` decides what those columns return when masked.
...
Expressions must use bare column names, not semantic `{model.column}` references. If a policy rule lists a column under `mask`, the physical model must define a matching `column_mask_expressions` entry.

### column\_classifications

Assign governance or catalog sensitivity labels to columns. This metadata is distinct from runtime masking and does not change query results.
