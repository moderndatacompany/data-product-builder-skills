...
This is a deterministic renderer: every value in the output is either derived from your project, defaulted, or explicitly set in the `deployment:` block in `config.yaml` — there are no placeholder values left for you to fill in by hand. See [Configurations](configurations/README.md#deployment) for the `deployment:` field reference.
...
Options:
  -o, --output TEXT  Output file path. Use "-" to write to stdout.
  --help             Show this message and exit.
```
...
Pipe the result straight into `dataos-ctl` instead of writing a file first:
...
`vulcan init` also adds `.vulcan/` to `.gitignore` — it holds the default local state database (see [State connection](configurations/README.md#state-connection)) and shouldn't be committed.
...
GraphQL is served in-process from this same server, at `/semantic/graphql` — there's no separate GraphQL command or process to start. Execution tunables are set via environment variables (`GRAPHQL_POLL_*`, `RETRY_AFTER`, `DEFAULT_TIMEZONE`); see [Configurations](configurations/README.md).
