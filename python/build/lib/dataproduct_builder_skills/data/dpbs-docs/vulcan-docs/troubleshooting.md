---
description: >-
  Every validation and request error Vulcan returns, what triggered it, and how
  to fix it.
---
...
Vulcan explicitly rejects values from `1` to `4` so you don't accidentally cache results for a too-short window.
...
Some semantic queries need a post-processing step after transpilation (for example, certain time grain rollups). When you set `disable_post_processing=true` on the request, Vulcan refuses to run those queries because the results would be wrong.
...
This usually shows up on internal semantic SQL flows where Vulcan needs to convert an incoming dialect (for example, snowflake) into the target backend dialect first, and that conversion failed.
...
These come from DataOS and the auth middleware. They tell you whether the request was authenticated, and whether the authenticated user is allowed to do what they asked.
...
DataOS received the request but determined the token is expired, malformed, revoked, or otherwise unusable.
...
DataOS timed out, returned invalid data, or wasn't reachable.

**Fix:**

* Check DataOS's health and logs.
* Verify network connectivity from the API service to DataOS.
* Retry after DataOS is restored.
...
<summary>Do I need to send query_type?</summary>

No. Query requests (source SQL, semantic SQL, REST, GraphQL, metric) are classified server-side into `query_type` at the entry point, and the state store persists a non-null value for it. This is internal bookkeeping — API clients never send `query_type` and don't need any request/response changes.
...
You declared the field without any semantic or physical role, so Vulcan doesn't know what to do with it.
...
You can only build a perspective from a statement that completed successfully and has a stored result. Vulcan rejects pending or failed statements.
...
See the Vulcan API Guide's polling section for the full status lifecycle and recommended poll behavior.
{% endhint %}
