MODEL (
  name LENOVO.USDK.software_geo_os_prod__@{rollup_type},
  kind FULL,
  owner 'shreyasikarwartmdcio',
  profiles (ROLLUP_TYPE, EVENT_DATE, ACTION, OS_FAMILY, PRODUCT_ID, REGION),
  tags ('gold', 'software-analytics', 'os-analytics', 'product-analytics', 'geographic-analytics', 'engagement-metrics', 'lenovo', 'usdk'),
  terms ('software_engagement', 'os_distribution', 'product_usage', 'geographic_distribution'),
  description 'Gold layer software analytics aggregated by OS family, product version, and geography. Tracks product usage across different Windows versions and regions. Essential for OS-product compatibility analysis, regional deployment strategies, and version adoption patterns.',
  grains (
    ROLLUP_TYPE, EVENT_DATE, ACTION, OS_FAMILY, PRODUCT_ID, PRODUCT_VERSION, REGION, COUNTRY_NAME
  ),
  blueprints (
    (
  ...
  column_descriptions (
    ...
    PRODUCT_VERSION = 'Software product version number',
...
/* Software geography, OS, and product analytics */
WITH flattened AS (
  SELECT
    *
