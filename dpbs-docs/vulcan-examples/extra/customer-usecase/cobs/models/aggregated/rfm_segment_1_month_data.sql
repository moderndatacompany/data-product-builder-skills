...
MODEL (
  name cobs.rfm_segment_1_month_data,
  kind INCREMENTAL_BY_TIME_RANGE (
    time_column date
  ),
  cron '@daily',
  grain (Account_ID, date),
  tags ('rfm', 'customer_segmentation', 'web_segment', 'sales_analytics'),
  terms ('customer.segmentation', 'rfm.analysis', 'sales.analytics'),
  description 'RFM (Recency, Frequency, Monetary) customer segmentation analysis based on 1-month rolling window. Delivers customer behavior insights including wallet share, order share, and proof transaction metrics for customer engagement and retention strategies',
  column_descriptions (
    ...
    date = 'Analysis date (start of 1-month window)'
  ),
  column_tags (
...
WITH params_filters AS (
  ...
  FROM (
    SELECT
      DATEADD(MONTH, -1, CURRENT_DATE())::DATE AS start_date,
