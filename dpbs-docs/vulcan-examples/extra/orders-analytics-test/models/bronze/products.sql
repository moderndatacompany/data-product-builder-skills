...
MODEL (
  name bronze_v1.products,
  kind SEED (
    path '../../seeds/seed_products.csv'
  ),
  columns (
    product_id INTEGER,
    supplier_id INTEGER,
  ...
  grains (product_id),
  tags ('dimension', 'product', 'inventory', 'pricing'),
  terms ('product.catalog', 'inventory.product'),
  ...
  column_tags (
    ...
    supplier_id = ('foreign_key', 'reference', 'supply_chain'),
    name = ('dimension', 'label'),
    category = ('dimension', 'classification'),
    price = ('measure', 'financial', 'metric')
  ),
  column_terms (
    product_id = ('product.product_id', 'inventory.product_id'),
    supplier_id = ('supplier.supplier_id', 'supply_chain.supplier_id'),
    name = ('product.name', 'inventory.product_name'),
    category = ('product.category', 'classification.product_category'),
    price = ('product.price', 'finance.unit_price')
  ),
  -- assertions (
  ...
  --   accepted_range(column := price, min_v := 0, max_v := 10000),
  --   forall(criteria := (
  --     product_id > 0,
  --     supplier_id > 0
  --   ))
  -- ),
  profiles (name, category, price, supplier_id)
);
