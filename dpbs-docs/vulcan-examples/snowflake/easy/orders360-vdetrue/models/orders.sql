MODEL (
  ...
  cron '@daily',
  grain ORDER_ID,
  tags ('fact', 'orders', 'sales', 'transactions', 'incremental', 'vde_enabled'),
  terms ('sales.orders', 'transactions.order_fact'),
  description 'Orders fact table loaded incrementally by ORDER_DATE. Captures the per-order transactional grain (quantity, unit price, discount, tax, shipping, total) for revenue analytics and joins back to DEMODB.VDETRUE.CUSTOMERS for dimensional slicing.',
  assertions (
    not_null(columns := (ORDER_ID, ORDER_DATE, CUSTOMER_ID))
  ),
  column_descriptions (
    ORDER_ID      = 'Unique identifier for each order',
    ORDER_DATE    = 'Timestamp when the order was placed (incremental time column)',
    CUSTOMER_ID   = 'Foreign key to DEMODB.VDETRUE.CUSTOMERS',
  ...
  column_tags (
    ...
    ORDER_DATE    = ('temporal', 'timestamp', 'time_column'),
...
WHERE ORDER_DATE::TIMESTAMP BETWEEN @start_ts AND @end_ts
