MODEL (
  name LENOVO.USDK.product_geo_cversion__@{rollup_type},
  kind FULL,
  owner 'shreyasikarwartmdcio',
  profiles (ROLLUP_TYPE, EVENT_DATE, ACTION, COMPONENT_VERSION, PRODUCT_ID, REGION),
  tags ('gold', 'product-analytics', 'version-analytics', 'geographic-analytics', 'engagement-metrics', 'cumulative-metrics', 'lenovo', 'usdk'),
  terms ('product_engagement', 'version_adoption', 'geographic_distribution', 'cumulative_metrics'),
  description 'Gold layer product analytics aggregated by component version and geography without component ID dimension. Provides simplified regional version adoption view with cumulative engagement metrics. Essential for high-level version distribution analysis across markets.',
  grains (
    ROLLUP_TYPE, EVENT_DATE, ACTION, COMPONENT_VERSION, PRODUCT_ID, REGION, COUNTRY_NAME
  ),
  blueprints (
    (
  ...
  column_descriptions (
    ...
    COMPONENT_VERSION = 'Version of the software component aggregated across all component IDs for simplified version analysis',
  ...
  column_tags (
    ...
    COMPONENT_VERSION = ('component-attribute', 'version', 'dimension', 'grain', 'aggregated'),
...
WITH flattened AS (
  ...
  SELECT
    ...
    SUM(COALESCE(n.daily_new_devices, 0)) OVER (
      PARTITION BY d.rollup_type, d.action, d.geo, d.component_version, d.product_id
