...
MODEL (
  name nilus.vulcan.qualtrics_questions_table_with_all_response_fields,
  ...
  terms ('qualtrics.enriched_question_metadata', 'survey.complete_field_catalog', 'dimension.master'),
  column_descriptions (
  ...
  column_terms (
    ...
    question_id = ('qualtrics.question_id', 'dimension.question'),
    questions_key = ('question.export_key', 'response.join_key'),
    ...
    data_export_struct = ('metadata.export_structure', 'schema.definition')
  ),
  physical_properties (
    format = 'iceberg'
  )
...
WITH response_with_survey_id AS (
  ...
  SELECT
    ...
    CASE
      ...
      THEN SPLIT(SUBSTRING_INDEX(SUBSTRING_INDEX(values_response, '[', -1), ']', 1), '#_ ')
      ELSE ARRAY(
        REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(values_response, '[', -1), ']', 1), '#_', ',')
      )
    END AS response_num_in_array,
    CASE
      WHEN NOT values_key LIKE '%_TEXT%'
      THEN SPLIT(
        REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(responses, '[', -1), ']', 1), '#+', ','),
        ','
      )
      ELSE ARRAY(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(responses, '[', -1), ']', 1), '#_', ','))
  ...
  SELECT
    ...
    question_id,
    question_number,
    NULLIF(CASE WHEN LOWER(row_id) = 'null' THEN NULL ELSE row_id END, '') AS row_id,
    ...
    NULLIF(CASE WHEN LOWER(srq_id) = 'null' THEN NULL ELSE srq_id END, '') AS srq_id,
    ...
    NULLIF(CASE WHEN LOWER(analyze_choice) = 'null' THEN NULL ELSE analyze_choice END, '') AS analyze_choice,
    NULLIF(
