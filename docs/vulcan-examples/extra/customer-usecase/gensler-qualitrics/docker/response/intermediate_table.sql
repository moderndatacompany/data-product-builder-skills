...
MODEL (
  name nilus.vulcan.qualtrics_response_intermediate_table,
  ...
  terms ('qualtrics.response_intermediate', 'response.fact', 'data_transformation.enriched'),
  column_descriptions (
  ...
  column_terms (
    ...
    questions_key = ('question.export_key', 'response.variable'),
    question_type = ('qualtrics.question_type', 'question.classification'),
    response = ('response.value', 'fact.measurement'),
...
WITH responses_for_each_question AS (
  SELECT
    ...
    b.response_type,
    b.response_value
  FROM nilus.vulcan.qualtrics_questions_table_with_all_response_fields AS a
  INNER JOIN nilus.vulcan.repartition_merging_survey_id_with_no_response_files AS b
    ON b.survey_id = a.survey_id
    AND (
      (
        b.values_key = 'QID_null' AND a.row_id = b.row_num
      )
      OR a.questions_key = b.values_key
    )
), first_case AS (
  SELECT
    survey_id,
  ...
  SELECT
    ...
    CASE
      ...
      WHEN (
        ...
        OR (
          choices RLIKE '.*[A-Za-z].*' AND choice_key = response
        )
      )
  ...
  SELECT
    ...
    (
      CASE
        WHEN response_value RLIKE '^\\d*$'
        THEN response_value
        WHEN response_value RLIKE '^\\d+\\.?\\d*$'
        THEN response_value
        ELSE NULL
      END
