...
MODEL (
  name GENSLER.FINAL.QUALTRICS_RESPONSE_RESPONDENT,
  ...
  terms ('respondent_metadata', 'completion_status', 'respondent'),
  column_descriptions (
  ...
  column_terms (
    ...
    progress = ('progress_percentage', 'metric'),
    user_language = ('language', 'code'),
    distribution_channel = ('channel', 'method'),
    end_date = ('end_timestamp', 'time'),
    start_date = ('start_timestamp', 'time'),
    duration = ('duration_seconds', 'measurement'),
    recorded_date = ('recorded_timestamp', 'timestamp')
  )
);
WITH unnesting_properties AS (
    SELECT
        ...
        f.value AS values1
    FROM
        GENSLER.RAW.QUALTRICS_RESPONSE_SCHEMA AS t,
        LATERAL FLATTEN(
...
response_table_with_variable_names AS (
    SELECT
        ...
        a.question_id,
        CASE
            WHEN b.variable_names IS NULL THEN a.questions_key
            ELSE b.variable_names
        ...
        a.sub_questions_key
    FROM
        GENSLER.RAW.QUALTRICS_RESPONSE_INTERMEDIATE_TABLE AS a
        FULL OUTER JOIN extracting_variable_names AS b ON b.survey_id = a.survey_id
...
response_table_null_questions_key_removed AS (
    SELECT
        ...
        question_id,
        CASE
            WHEN questions_key IS NULL THEN question_id
            ELSE questions_key
        END AS questions_key,
        question_number,
        question_name,
        ...
        sub_questions_key
    FROM
        response_table_with_variable_names
),
filtering_repeated_choice_text_rows AS (
    SELECT
        *
    FROM
        response_table_null_questions_key_removed
    -- WHERE
    --     CASE
    --         WHEN (
    ...
    --             OR CASE
    --                 WHEN choice_recode IS NULL THEN choice_key
    --                 ELSE choice_recode
    --             END::INT = response_value
    --         ) THEN 1
    --         ELSE 2
    --     END IN (0, 1)
),
respondent_metadata_table AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN questions_key = 'status' THEN response
            END
        ) AS status,
        MAX(
            CASE
                WHEN questions_key = 'finished' THEN response
            END
        )::BOOLEAN AS finished,
        MAX(
            CASE
                WHEN questions_key = 'progress' THEN response
            END
        )::INT AS progress,
        MAX(
            CASE
                WHEN questions_key = 'userLanguage' THEN response
            END
        ) AS user_language,
        MAX(
            CASE
                WHEN questions_key = 'distributionChannel' THEN response
            END
        ) AS distribution_channel,
        MAX(
            CASE
                WHEN questions_key = 'endDate' THEN response
            END
        )::TIMESTAMP AS end_date,
        MAX(
            CASE
                WHEN questions_key = 'startDate' THEN response
            END
        )::TIMESTAMP AS start_date,
        MAX(
            CASE
                WHEN questions_key = 'duration' THEN response
            END
        )::INT AS duration,
        MAX(
            CASE
                WHEN questions_key = 'recordedDate' THEN response
            END
        )::TIMESTAMP AS recorded_date
    FROM
        GENSLER.RAW.QUALTRICS_RESPONSE_INTERMEDIATE_TABLE
    -- WHERE
    --     questions_key IN (
    --         'status',
    --         'finished',
