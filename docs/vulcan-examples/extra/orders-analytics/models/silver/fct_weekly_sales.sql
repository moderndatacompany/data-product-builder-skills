...
MODEL (
  name silver_v2alpha.fct_weekly_sales,
  kind FULL,
  cron '*/5 * * * *',
  grains (week_start_date, region_id, customer_id, product_id),
  tags ('fact', 'aggregated', 'sales', 'weekly', 'silver'),
  terms ('sales.weekly_metrics', 'analytics.weekly_sales'),
  ...
  column_tags (
    ...
    avg_daily_revenue = ('measure', 'financial', 'metric'),
    total_shipments = ('measure', 'metric', 'count'),
    shipment_rate = ('measure', 'metric', 'percentage')
  ),
  column_terms (
    week_start_date = ('time.week_start', 'calendar.week_start_date'),
  ...
  assertions (
    unique_combination_of_columns(columns := (week_start_date, region_id, customer_id, product_id)),
    not_null(columns := (week_start_date, week_end_date, year, week_number, region_id, region_name, customer_id, product_id)),
    forall(criteria := (
      total_orders >= 0,
      total_items_sold >= 0,
      total_revenue >= 0,
    ...
    accepted_range(column := shipment_rate, min_v := 0, max_v := 1),
    accepted_range(column := week_number, min_v := 1, max_v := 53)
  ),
  assertions (
    not_null(columns := (week_start_date, week_end_date, year, week_number, region_id, region_name, customer_id, product_id)),
    forall(criteria := (
  ...
  profiles (region_name, customer_id, product_id, total_orders, total_revenue, avg_order_value, avg_daily_revenue)
);
...
WITH order_metrics AS (
  ...
  INNER JOIN bronze_v2alpha.order_items AS oi
