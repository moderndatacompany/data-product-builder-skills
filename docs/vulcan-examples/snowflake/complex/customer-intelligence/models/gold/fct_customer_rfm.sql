...
WITH customer_orders AS (
  -- Aggregate order metrics per customer (last 365 days)
  SELECT
    o."customer_id"::VARCHAR(50) AS customer_id,
    DATEDIFF(DAY, MAX(o."order_date"::DATE), CURRENT_DATE()) AS recency_days,
  ...
  WHERE o."order_status" IN ('Confirmed', 'Shipped', 'Invoiced')
    AND o."order_date"::DATE >= DATEADD(DAY, -365, CURRENT_DATE())
...
rfm_scored AS (
  -- Calculate RFM scores using NTILE for even distribution (1-5 scale)
  SELECT
    *,
    ...
    -- Monetary: Higher value = better (score 5)
    CASE
...
rfm_with_composite AS (
  -- Build composite RFM score string
  SELECT
    *,
    CONCAT(recency_score::VARCHAR, frequency_score::VARCHAR, monetary_score::VARCHAR) AS rfm_score
  FROM rfm_scored
),

rfm_segmented AS (
  -- Assign segment based on RFM thresholds from seed data
  SELECT
    r.customer_id,
    r.customer_name,
    r.customer_type,
    ...
    -- Row number to pick the best matching segment (most specific match)
    ROW_NUMBER() OVER (
