...
WITH customer_rfm_metrics AS (
  -- Leverage silver layer customer profile with pre-calculated metrics
  SELECT
    cp.customer_id,
    cp.customer_name,
    cp.email,
    cp.region_name,
    cp.days_since_last_order AS recency_days,
    cp.total_orders AS frequency_orders,
    cp.total_revenue AS monetary_value
