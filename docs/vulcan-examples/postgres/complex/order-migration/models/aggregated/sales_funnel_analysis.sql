...
MODEL (
    name aggregated.sales_funnel_analysis,
    kind FULL,
    cron '*/5 * * * *',
    ...
    column_tags (
        ...
        drop_off_after_registration = (
            'measure',
            'metric',
            'drop_off'
        ),
        drop_off_orders_without_items = (
            'measure',
            'metric',
            'drop_off'
        ),
        drop_off_orders_not_shipped = (
            'measure',
            'metric',
            'drop_off'
        ),
    ...
    column_terms (
        ...
        overall_conversion_rate = (
            'analytics.overall_conversion',
            'funnel.end_to_end_conversion'
        )
    ),
    assertions (
        unique_combination_of_columns (
            columns := (funnel_date, region_id)
        ),
        not_null (
            columns := (
                funnel_date,
        ...
        accepted_range (
            column := conversion_rate_registration_to_order,
            min_v := 0,
            max_v := 1
        ),
        accepted_range (
            column := conversion_rate_order_to_items,
            min_v := 0,
            max_v := 1
        ),
        accepted_range (
            column := conversion_rate_order_to_shipment,
            min_v := 0,
            max_v := 1
        )
    ),
...
WITH
    ...
    stage_2_ordering_customers AS (
        ...
        SELECT c.region_id, COUNT(DISTINCT c.customer_id) AS customers_with_orders
        from raw.customers AS c
            INNER JOIN bronze.orders AS o ON c.customer_id = o.customer_id
        GROUP BY
            c.region_id
    ),
    stage_3_orders AS (
        ...
        SELECT
            ...
            AVG(
                CASE
                    WHEN s.shipped_date IS NOT NULL THEN DATE(s.shipped_date) - DATE(o.order_date)
                    ELSE NULL
                END
            ) AS avg_time_to_ship_days
    ...
    stage_5_shipped_orders AS (
        ...
        SELECT c.region_id, COUNT(DISTINCT s.order_id) AS orders_shipped
        from raw.shipments AS s
            INNER JOIN bronze.orders AS o ON s.order_id = o.order_id
            INNER JOIN bronze.customers AS c ON o.customer_id = c.customer_id
        GROUP BY
            c.region_id
    )
SELECT CAST(s1.funnel_date AS TIMESTAMP) AS funnel_date, s1.region_id, s1.region_name,
...
-- Additional Metrics
ROUND(
    COALESCE(s4.avg_items_per_order, 0),
    2
) AS avg_items_per_order,
ROUND(
    COALESCE(s3.avg_time_to_ship_days, 0),
    2
) AS avg_time_to_ship_days
FROM
    stage_1_customers AS s1
    LEFT JOIN stage_2_ordering_customers AS s2 ON s1.region_id = s2.region_id
    LEFT JOIN stage_3_orders AS s3 ON s1.region_id = s3.region_id
    LEFT JOIN stage_4_orders_with_items AS s4 ON s1.region_id = s4.region_id
    LEFT JOIN stage_5_shipped_orders AS s5 ON s1.region_id = s5.region_id